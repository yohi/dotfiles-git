...
if grep -q "set -o pipefail" "${SCRIPT_DIR}/_scripts/lazygit-ai-commit/ai-commit-generator.sh"; then
...
if grep -q "set -o pipefail" "${SCRIPT_DIR}/_scripts/lazygit-ai-commit/parse-ai-output.sh"; then
...
if grep -q "timeout.*TIMEOUT_SECONDS" "${SCRIPT_DIR}/_scripts/lazygit-ai-commit/ai-commit-generator.sh"; then
...
if grep -q "AI tool timed out" "${SCRIPT_DIR}/_scripts/lazygit-ai-commit/ai-commit-generator.sh"; then
...
OUTPUT=$(echo -e "feat: add feature\nfix: fix bug" | "${SCRIPT_DIR}/_scripts/lazygit-ai-commit/parse-ai-output.sh" 2>&1)
...
ERROR_COUNT=$(grep -c "Suggestion:" "${SCRIPT_DIR}/_scripts/lazygit-ai-commit/ai-commit-generator.sh" || true)
...
if grep -q "set -o pipefail" "${SCRIPT_DIR}/config.yml"; then
