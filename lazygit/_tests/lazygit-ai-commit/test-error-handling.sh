...
if grep -q "set -o pipefail" ../../_scripts/lazygit-ai-commit/ai-commit-generator.sh; then
...
if grep -q "set -o pipefail" ../../_scripts/lazygit-ai-commit/parse-ai-output.sh; then
...
if grep -q "timeout.*TIMEOUT_SECONDS" ../../_scripts/lazygit-ai-commit/ai-commit-generator.sh; then
...
if grep -q "AI tool timed out" ../../_scripts/lazygit-ai-commit/ai-commit-generator.sh; then
...
# Backup and replace mock-ai-tool.sh temporarily
    ...
    cp "$TEMP_SCRIPT" ../../_scripts/lazygit-ai-commit/mock-ai-tool.sh
...
OUTPUT=$(echo -e "feat: add feature\nfix: fix bug" | ../../_scripts/lazygit-ai-commit/parse-ai-output.sh 2>&1)
...
ERROR_COUNT=$(grep -c "Suggestion:" ../../_scripts/lazygit-ai-commit/ai-commit-generator.sh || true)
